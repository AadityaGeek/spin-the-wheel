...
const colors = [
    "#FFD700", "#FF6347", "#6A5ACD", "#32CD32", "#FF69B4", "#4682B4", "#DAA520", "#BA55D3",
    "#F08080", "#20B2AA", "#7B68EE", "#ADFF2F", "#FF4500", "#1E90FF", "#8A2BE2", "#F4A460"
];
...
function drawWheel() {
    ...
    if (options.length === 0) {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (textWidth > outsideRadius * 1.5) {
                let maxChars = Math.floor(text.length * (outsideRadius * 1.5) / textWidth);
    ...
    ctx.beginPath();
    ctx.arc(canvas.width / 2, canvas.height / 2, insideRadius + 20, 0, 2 * Math.PI, false); // Smaller center circle
    ...
    options.forEach((option, index) => {
        ...
        let probability = (totalWeight > 0) ? ((option.weight / totalWeight) * 100).toFixed(1) : 0; // Calculate probability
        ...
        displaySpan.className = 'text-gray-700 cursor-pointer hover:underline editable-option-display';
        ...
        displaySpan.addEventListener('click', (event) => {
            ...
            const saveChanges = () => {
                ...
                if (isNaN(newWeight) || newWeight < 0) {
                    showMessageModal("Invalid Weight", "Please enter a non-negative number for the weight.", true);
        ...
        removeButton.className = 'remove-option-btn text-gray-400 hover:text-red-500 transition-colors duration-200';
        ...
        removeButton.addEventListener('click', (event) => {
            ...
            if (!isNaN(idxToRemove) && idxToRemove >= 0 && idxToRemove < options.length) {
...
addOptionBtn.addEventListener('click', () => {
    ...
    if (newOptionText && options.length < 16) { // Limit options to prevent clutter
        ...
        showMessageModal("Limit Reached", "You've reached the maximum number of options (16).", true);
    } else {
...
function animateSpin() {
    ...
    if (spinTime >= spinTimeTotal) {
        ...
        currentRotationAngle = initialSpinAngle + (spinArcTargetDegrees * Math.PI / 180);
        stopRotateWheel();
        return;
    }

    // Calculate the current rotation offset using easeOut
    const currentSpinOffsetDegrees = easeOut(spinTime, 0, spinArcTargetDegrees, spinTimeTotal);
    currentRotationAngle = initialSpinAngle + (currentSpinOffsetDegrees * Math.PI / 180);
...
spinBtn.addEventListener('click', () => {
    if (options.length === 0) {
        showMessageModal("No Options", "Please add at least one option to spin the wheel!", true);
        return;
    }
    ...
    const targetSegmentCenterAngleFromWheelZero = preSelectedWinningOptionIndex * arc + arc / 2;
    ...
    let requiredCurrentRotationAngleAtStop = (3 * Math.PI / 2) - targetSegmentCenterAngleFromWheelZero;

    // Normalize `requiredCurrentRotationAngleAtStop` to be between 0 and 2*Math.PI
    requiredCurrentRotationAngleAtStop = (requiredCurrentRotationAngleAtStop % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI);
    ...
    let currentNormalizedAngle = (currentRotationAngle % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI);
    
    // Calculate the angular difference to reach the target within one revolution
    let angularDifference = requiredCurrentRotationAngleAtStop - currentNormalizedAngle;
    ...
    const totalSpinRadians = (minFullRotations + randomFullRotations) * 2 * Math.PI + angularDifference;
